  ...
  def _create_predictions(self, decoder_output, features, labels, losses=None):
    ...
    if "predicted_ids" in predictions.keys():
      ...
      predicted_tokens = target_id_to_vocab.lookup(
          tf.to_int64(predictions["predicted_ids"]), name="predicted_tokens")
      predicted_tokens_0 = tf.gather_nd(predicted_tokens, indices=[0], name="predicted_tokens_0")
      predicted_tokens_scalar = tf.reduce_join(predicted_tokens, separator=' ', name="predicted_tokens_scalar")
      if "beam_search_output.beam_parent_ids" in predictions.keys():
        beam_parent_ids = tf.gather_nd(predictions["beam_search_output.beam_parent_ids"], indices=[0], name="beam_parent_ids")
        predictions["beam_parent_ids"] = beam_parent_ids
      # Raw predicted tokens
      predictions["predicted_tokens"] = predicted_tokens
      predictions["predicted_tokens_scalar"] = predicted_tokens_scalar
      beam_predicted_ids = tf.gather_nd(predictions["predicted_ids"], indices=[0], name="beam_predicted_ids")
      predictions["beam_predicted_ids"] = beam_predicted_ids
