...
class TripleTextInputPipeline(InputPipeline):
  """An input pipeline that reads three parallel (line-by-line aligned) text
  files.

  Params:
    query_files: An array of file names for the query data.
    candidate_files: An array of file names for the candidate answers data.
    target_files: An array of file names for the target data. These must
      be aligned to the `source_files`.
    source_delimiter: A character to split the source text on. Defaults
      to  " " (space). For character-level training this can be set to the
  ...
  @property
  def feature_keys(self):
    return set(["source_tokens", "source_len", "source_candidate_tokens", "source_candidate_len"])
