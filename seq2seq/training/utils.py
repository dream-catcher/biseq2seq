...
def create_input_fn(pipeline,
  ...
  def input_fn():
    ...
    with tf.variable_scope(scope or "input_fn"):
      ...
      print("data_provider:{} features_and_labels:{}".format(data_provider, features_and_labels))
      ...
      print("features_batch:{} labels_batch:{}".format(features_batch, labels_batch))
      return features_batch, labels_batch

  return input_fn


def create_seq2seq_input_fn(pipeline,
                    batch_size,
                    bucket_boundaries=None,
                    allow_smaller_final_batch=False,
                    scope=None):
  ...
  def input_fn():
    ...
    with tf.variable_scope(scope or "input_fn"):
      ...
      #merge biseq2seq query & candidate inputs
      #features_batch = {k: batch[k] for k in pipeline.feature_keys}
      source_tokens = tf.concat([batch["source_tokens"], batch["source_candidate_tokens"]], 0)
      source_len = tf.concat([batch["source_len"], batch["source_candidate_len"]], 0)
      features_batch = {"source_tokens":source_tokens, "source_len":source_len}
      print("source_tokens:{}".format(source_tokens))
      print("source_len:{}".format(source_len))
      print("features_batch:{}".format(features_batch))
