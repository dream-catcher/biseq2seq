...
class ExportSavedModelHook(TrainingHook):
  """A hook to export saved model format at end.
     Model file use to inference in java languade.
  """

  def __init__(self, params, model_dir, run_config):
    super(ExportSavedModelHook, self).__init__(params, model_dir, run_config)
    self._output_dir = os.path.abspath(os.path.join(self.model_dir, "fin_biseq2seq_model"))
    self._builder = tf.saved_model.builder.SavedModelBuilder(self._output_dir)
  ...
  def begin(self):
    ...
    with tf.Session(graph=tf.Graph()) as sess:
      self._builder.add_meta_graph_and_variables(sess, ["fin_biseq2seq"])
