...
def customize_att_sum_dot(keys, query):
  """Calculates a batch- and timweise dot product"""
  return tf.reduce_sum(keys * tf.expand_dims(query, 1), [2], name="customize_att_sum_dot")
...
class CustomizeAttentionLayerDot(AttentionLayer):
  """An attention layer that calculates attention scores using
  a dot product.
  """

  def score_fn(self, keys, query):
    return customize_att_sum_dot(keys, query)
