...
COPY_FILE_LIST = ["checkpoint",
                "graph.pbtxt",
                "model_analysis.txt",
                "run_meta",
                "train_options.json"]

COPY_PATTERN_LIST = ["model\.ckpt-.*"]
...
tf.flags.DEFINE_integer("keep_checkpoint_max", 1,
...
tf.flags.DEFINE_string("model_output_path", None,
                       """The directory to write model checkpoints and summaries
                       to. If None, a local temporary directory is created.""")
...
def create_experiment(output_dir):
  ...
  print("start to create experiment")
  ...
  print("train_input_pipeline:{}".format(train_input_pipeline))
  ...
  print("train_input_fn:{}".format(train_input_fn))
  ...
  print("dev_input_pipeline:{}".format(dev_input_pipeline))
  ...
  print("eval_input_fn:{}".format(eval_input_fn))
  ...
  print("finish creating experiment:{}".format(experiment))
  return experiment
...
def main(_argv):
  ...
  if not conf.IN_LOCAL_ENV:
     if not hdfs.path.exists(FLAGS.model_output_path):
         ...
         print("dir:{} exists:{}".format(FLAGS.model_output_path, hdfs.path.exists(FLAGS.model_output_path)))
     print("copying files from {} to hdfs:{}".format(FLAGS.output_dir, FLAGS.model_output_path))
     output_files = os.listdir(FLAGS.output_dir)
     for f in output_files:
        if f in COPY_FILE_LIST:
            print("copy file:{} to hdfs:{}".format(f, FLAGS.model_output_path))
            pydoop.hdfs.put(os.path.join(FLAGS.output_dir, f), FLAGS.model_output_path)
        else:
